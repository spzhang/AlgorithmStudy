...
class SearchA2DMatrixSolution {
    ...
    boolean searchAux(int[][] matrix, int s, int e, int target){
        ...
        int mid = (s+e) / 2;
        if(mid-1 < s){
            if(s==e) return searchRow(matrix[s], 0, cols-1, target);
            else if(target > matrix[s][cols-1]){
                return searchRow(matrix[e], 0, cols-1, target);
            }
            else return searchRow(matrix[s], 0, cols-1, target);
        }
        else{
            ...
            else return searchRow(matrix[mid], 0, cols-1, target);
        }
    }
    ...
    boolean searchRow(int[] A, int s, int e, int target){
        ...
        if(mid - 1 < s){
            ...
            else if (A[e] == target) return true;
            else return false;
        }
        else{
            if(A[mid] == target) return true;
            else if (A[mid] > target) return searchRow(A, s, mid-1, target);
            else return searchRow(A, mid+1, e, target);
        }
    }
}